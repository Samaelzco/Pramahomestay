  ...
  return (
    <form action={action} className="mt-10 space-y-6">
      <div>
        ...
        {state.errors?.email?.map((error) => <p key={error} className="mt-2 text-sm text-danger">{error}</p>)}
      </div>
      <div>
        <label htmlFor="password" className="mb-2 block text-xs font-semibold tracking-[0.1em] text-muted uppercase">Password</label>
      ...
      {state.message && !state.errors?.email && <p role="alert" className="rounded-sm bg-[#ffdad6] px-4 py-3 text-sm text-[#93000a]">{state.message}</p>}
      <button type="submit" disabled={pending} className="flex h-12 w-full items-center justify-center rounded-sm bg-primary px-5 text-sm font-semibold text-white transition-colors hover:bg-[#2f3131] disabled:opacity-60">
        {pending ? "Memeriksa akses…" : "Masuk ke area internal"}
      </button>
    </form>
  );
}
