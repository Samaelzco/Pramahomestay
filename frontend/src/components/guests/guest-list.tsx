  ...
  return <div className="mt-5 overflow-hidden rounded-lg bg-surface shadow-[0_18px_42px_-28px_rgba(68,71,72,0.25)]">
    ...
    <div className="divide-y">{guests.map((guest) => <article key={guest.id} className="grid gap-5 px-5 py-6 transition-colors hover:bg-surface-low/60 sm:grid-cols-2 lg:grid-cols-[1.15fr_1.25fr_0.75fr_0.9fr_0.85fr_auto] lg:items-center lg:px-6">
      ...
      <div className="flex flex-wrap items-center gap-x-4 lg:flex-col lg:items-start">
        ...
        <ConfirmAction action={setGuestActivationAction.bind(null, guest.id, !guest.is_active)} trigger={guest.is_active ? localize(locale, "Nonaktifkan", "Deactivate") : localize(locale, "Aktifkan", "Activate")} title={guest.is_active ? localize(locale, `Nonaktifkan ${guest.full_name}?`, `Deactivate ${guest.full_name}?`) : localize(locale, `Aktifkan ${guest.full_name}?`, `Activate ${guest.full_name}?`)} description={guest.is_active ? localize(locale, "Profil tidak dapat dipilih untuk booking baru. Riwayat dan booking yang sudah ada tetap tersimpan.", "The profile cannot be selected for new bookings. Existing history and bookings remain stored.") : localize(locale, "Profil kembali dapat dipilih saat membuat booking baru.", "The profile can be selected for new bookings again.")} confirmLabel={guest.is_active ? localize(locale, "Ya, nonaktifkan", "Yes, deactivate") : localize(locale, "Ya, aktifkan", "Yes, activate")} tone={guest.is_active ? "danger" : "primary"} />
