...
export function RoomDetailGallery({ images, roomName, locale }: { images: GalleryImage[]; roomName: string; locale: Locale }) {
  ...
  return <>
    <section aria-label={localize(locale, `Galeri ${roomName}`, `${roomName} gallery`)} className={`relative grid gap-2 overflow-hidden sm:grid-cols-2 sm:gap-3 lg:h-[38rem] lg:grid-cols-12 lg:grid-rows-2 ${visibleImages.length === 1 ? "lg:block" : ""}`}>
      {visibleImages.map((image, index) => {
        const remaining = images.length - visibleImages.length;
        return <button key={image.id} type="button" onClick={() => setActiveIndex(index)} className={`group relative min-h-52 overflow-hidden bg-surface-high text-left ${index === 0 ? "sm:col-span-2 sm:min-h-[28rem] lg:col-span-8 lg:row-span-2 lg:min-h-0" : `hidden sm:block sm:min-h-56 lg:col-span-4 lg:min-h-0 ${visibleImages.length === 2 ? "sm:col-span-2 lg:col-span-4 lg:row-span-2" : ""}`} ${visibleImages.length === 1 ? "h-full w-full" : ""}`} aria-label={localize(locale, `Buka foto ${index + 1}`, `Open photo ${index + 1}`)}>
      ...
      <button type="button" onClick={() => setActiveIndex(0)} className="absolute right-4 bottom-4 flex min-h-11 items-center gap-2 bg-background px-4 text-sm font-bold text-foreground shadow-[0_16px_40px_-24px_rgba(0,0,0,0.6)] sm:right-5 sm:bottom-5"><ExpandIcon className="size-4" />{localize(locale, `Lihat ${images.length} foto`, `View ${images.length} photos`)}</button>
    </section>
