...
import { BedIcon, CalendarIcon, GridIcon, HomeIcon, LogOutIcon, ShieldIcon, UsersIcon, WalletIcon, XIcon } from "@/components/ui/icons";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { useEffect, useRef, type KeyboardEvent } from "react";

const navItems = [
  { label: "Ringkasan", href: "/internal/dashboard", icon: GridIcon, permission: "dashboard.view" },
  { label: "Booking", href: "/internal/bookings", icon: CalendarIcon, permission: "bookings.view" },
...
function InternalNavigation({ permissions, onNavigate }: { permissions: string[]; onNavigate?: () => void }) {
  ...
  return <nav aria-label="Navigasi internal"><ul className="space-y-1">{navItems.filter((item) => permissions.includes(item.permission)).map(({ label, href, icon: NavIcon }) => {
