...
function InternalNavigation({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <nav aria-label="Navigasi internal"><ul className="space-y-1">{navItems.map(({ label, href, icon: NavIcon, disabled }) => {
    const active = !disabled && pathname.startsWith(href);
    return <li key={label}>{disabled ? (
...
export function MobileSidebar({ open, onClose, userName }: { open: boolean; onClose: () => void; userName: string }) {
  ...
  return <div aria-hidden={!open} className={`fixed inset-0 z-50 md:hidden ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
    ...
    <aside ref={dialogRef} inert={!open} role="dialog" aria-modal="true" aria-label="Menu navigasi" onKeyDown={handleKeyDown} className={`absolute inset-y-0 left-0 flex w-[min(320px,86vw)] flex-col bg-surface px-4 py-6 shadow-[18px_0_48px_-24px_rgba(17,17,17,0.45)] transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none ${open ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex items-center justify-between gap-4 px-3"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-md bg-primary text-white"><HomeIcon className="size-5" /></span><div><p className="font-semibold tracking-[-0.02em]">Prama Homestay</p><p className="mt-1 text-[10px] font-semibold tracking-[0.12em] text-muted uppercase">Management</p></div></div><button ref={closeRef} type="button" onClick={onClose} aria-label="Tutup menu" className="grid size-11 shrink-0 place-items-center rounded-sm text-muted transition-colors hover:bg-surface-low hover:text-primary active:bg-surface-high"><XIcon className="size-5" /></button></div>
