    ...
    <form action={formAction} className="mt-10 max-w-5xl">
      ...
      <section className="grid gap-6 border-t py-8 lg:grid-cols-[220px_1fr]">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <label className="text-xs font-semibold tracking-[0.08em] text-muted uppercase sm:col-span-2">Kamar<select name="room_id" required value={roomId} onChange={(event) => setRoomId(event.target.value)} className={inputClass}>{rooms.map((item) => <option key={item.id} value={item.id}>{item.name} · maks. {item.capacity} tamu</option>)}</select><FieldError errors={state.errors?.room_id} /></label>
