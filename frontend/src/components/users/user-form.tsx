...
import type { ActionState, InternalUser } from "@/lib/api/types";
...
export function UserForm({ user }: { user?: InternalUser }) {
  ...
  return <form action={formAction} className="mt-10 max-w-4xl">
    ...
    <section className="grid gap-6 border-t py-8 md:grid-cols-[220px_1fr]"><div><h2 className="text-lg font-semibold">Role dan akses</h2><p className="mt-2 text-sm leading-6 text-muted">Administrator memiliki akses penuh. Staff mengikuti konfigurasi pada halaman Hak akses.</p></div><div className="grid gap-6 sm:grid-cols-2">
      <label className="text-xs font-semibold tracking-[0.08em] text-muted uppercase">Role{user?.is_self && <input type="hidden" name="role" value={user.roles[0]} />}<select name={user?.is_self ? undefined : "role"} required disabled={user?.is_self} defaultValue={user?.roles[0] ?? "staff"} className={`${inputClass} disabled:cursor-not-allowed disabled:bg-surface-low disabled:text-muted`}><option value="staff">Staff</option><option value="admin">Administrator</option></select><FieldError errors={state.errors?.role} /></label>
      {!user && <label className="flex min-h-12 items-center gap-3 self-end rounded-sm border bg-surface px-4 text-sm font-medium normal-case tracking-normal"><input type="checkbox" name="is_active" value="1" defaultChecked className="size-4 accent-black" />Aktifkan akses login</label>}
    </div></section>
