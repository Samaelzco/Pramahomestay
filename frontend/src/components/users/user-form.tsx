  ...
  return <form action={formAction} className="mt-10 max-w-4xl">
    ...
    <section className="grid gap-6 border-t py-8 md:grid-cols-[220px_1fr]"><div><h2 className="text-lg font-semibold">{localize(locale, "Role dan akses", "Role and access")}</h2><p className="mt-2 text-sm leading-6 text-muted">{localize(locale, "Administrator memiliki akses penuh. Role lainnya mengikuti permission pada halaman Hak akses.", "Administrators have full access. Other roles follow the permissions configured on the Access page.")}</p></div><div className="grid gap-6 sm:grid-cols-2">
      <div className="grid gap-3">
        ...
        <label className="flex min-h-16 items-start gap-3 rounded-sm border bg-surface px-4 py-3 text-sm font-medium normal-case tracking-normal transition-colors hover:bg-surface-low sm:col-span-2">
          <input type="checkbox" name="receives_internal_email_notifications" value="1" defaultChecked={user?.receives_internal_email_notifications ?? false} className="mt-0.5 size-4 accent-secondary" />
          <span><span className="block font-semibold">{localize(locale, "Terima notifikasi melalui email", "Receive email notifications")}</span><span className="mt-1 block font-normal leading-5 text-muted">{localize(locale, "Kabar penting sesuai hak akses akan dikirim ke email login user ini.", "Critical updates matching this user's permissions will be sent to their login email.")}</span></span>
        </label>
      </div>
    </div></section>
    ...
    {user?.is_self && <p className="border-t py-5 text-sm leading-6 text-muted">{localize(locale, "Anda dapat mengubah identitas dan password akun ini, tetapi tidak dapat mengganti role atau menonaktifkan akun sendiri.", "You can update this account's identity and password, but cannot change its role or deactivate your own account.")}</p>}
