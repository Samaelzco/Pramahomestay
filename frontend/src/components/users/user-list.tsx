...
export function UserList({ users }: { users: InternalUser[] }) {
  ...
  return <div className="mt-5 overflow-hidden rounded-lg bg-surface shadow-[0_18px_42px_-28px_rgba(68,71,72,0.25)]">
    ...
    <div className="divide-y">{users.map((user) => <article key={user.id} className="grid gap-5 px-5 py-6 transition-colors hover:bg-surface-low/60 sm:grid-cols-2 lg:grid-cols-[1.3fr_0.75fr_0.75fr_1fr_0.85fr_auto] lg:items-center lg:px-6">
      ...
      <div><p className="text-[10px] font-semibold tracking-[0.09em] text-muted uppercase lg:hidden">Role</p><span className="mt-1 inline-flex rounded-sm bg-secondary-soft px-2.5 py-1.5 text-[10px] font-semibold tracking-[0.08em] text-[#5f411b] uppercase lg:mt-0">{roleLabel(user.roles[0], user.role_labels[0] ?? user.roles[0], locale)}</span></div>
      ...
      <div><p className="text-[10px] font-semibold tracking-[0.09em] text-muted uppercase lg:hidden">{localize(locale, "Dibuat", "Created")}</p><p className="mt-1 text-sm tabular-nums lg:mt-0">{date.format(new Date(user.created_at))}</p></div>
      <div className="flex flex-wrap items-center gap-x-4 lg:flex-col lg:items-start">
        ...
        <ConfirmAction action={setUserActivationAction.bind(null, user.id, !user.is_active)} trigger={user.is_active ? localize(locale, "Nonaktifkan", "Deactivate") : localize(locale, "Aktifkan", "Activate")} title={user.is_active ? localize(locale, `Nonaktifkan ${user.name}?`, `Deactivate ${user.name}?`) : localize(locale, `Aktifkan ${user.name}?`, `Activate ${user.name}?`)} description={user.is_active ? localize(locale, "Akses login dihentikan dan seluruh token aktif dicabut. Data operasional tetap tersimpan.", "Login access will stop and all active tokens will be revoked. Operational data remains stored.") : localize(locale, "User kembali dapat login sesuai role dan hak aksesnya.", "The user can sign in again according to their role and permissions.")} confirmLabel={user.is_active ? localize(locale, "Ya, nonaktifkan", "Yes, deactivate") : localize(locale, "Ya, aktifkan", "Yes, activate")} tone={user.is_active ? "danger" : "primary"} disabled={!user.can_change_status} />
