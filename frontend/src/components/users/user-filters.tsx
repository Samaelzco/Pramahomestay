...
export function UserFilters({ search, role, isActive, roles }: { search?: string; role?: string; isActive?: string; roles: AccessRole[] }) {
  return <form className="mt-10 grid gap-3 border-y py-6 sm:grid-cols-2 lg:grid-cols-[minmax(260px,1fr)_190px_190px_auto]">
    ...
    <label><span className="sr-only">Filter role</span><select name="role" defaultValue={role ?? ""} className="h-12 w-full rounded-sm border bg-surface px-4 text-sm outline-none focus:border-primary"><option value="">Semua role</option>{roles.map((item) => <option key={item.name} value={item.name}>{item.label}</option>)}</select></label>
