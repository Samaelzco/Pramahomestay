  ...
  return <div className="sm:col-span-2">
    ...
    <div className="mt-2 grid overflow-hidden rounded-lg bg-surface-low sm:grid-cols-[1fr_180px]">
      <div className="flex min-h-48 flex-col items-start justify-center p-6">
        ...
        <p className="mt-4 text-base font-semibold">Logo properti</p>
        <p className="mt-2 max-w-md text-sm leading-6 text-muted">Gunakan JPG, PNG, atau WebP hingga 3 MB. Gambar persegi dengan latar transparan akan tampil paling baik.</p>
        <label className="mt-4 inline-flex min-h-11 cursor-pointer items-center rounded-sm border bg-surface px-5 text-sm font-semibold transition-colors hover:bg-surface-high focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-primary"><span>{previewUrl ? "Ganti logo" : "Pilih logo"}</span><input ref={inputRef} name="logo" type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => selectLogo(event.target.files?.[0])} /></label>
        <p className="mt-3 text-xs text-muted">{fileName ?? (previewUrl ? "Logo yang tersimpan saat ini" : "Belum ada logo")}</p>
      ...
      <div className="relative grid min-h-48 place-items-center bg-surface-high p-6">
        {previewUrl ? <><div className="relative size-28 overflow-hidden rounded-lg bg-surface"><Image src={previewUrl} alt="Preview logo Prama Homestay" fill sizes="112px" unoptimized={previewUrl.startsWith("blob:")} className="object-contain p-2" /></div><button type="button" onClick={clearLogo} className="absolute top-3 right-3 inline-flex min-h-10 items-center gap-2 rounded-sm bg-primary px-3 text-xs font-semibold text-white"><TrashIcon className="size-4" />Hapus</button></> : <p className="text-center text-sm leading-6 text-muted">Preview logo</p>}
      </div>
    </div>
  </div>;
}
