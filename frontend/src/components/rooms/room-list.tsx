      ...
      <div className="divide-y">
        {rooms.map((room) => (
          <article key={room.id} className="grid gap-5 px-5 py-6 transition-colors hover:bg-surface-low/60 sm:grid-cols-2 lg:grid-cols-[1.2fr_0.9fr_1.2fr_0.8fr_0.9fr_auto] lg:items-center lg:px-6">
            ...
            <div><p className="text-[10px] font-semibold tracking-[0.09em] text-muted uppercase lg:hidden">Fasilitas</p><p className="mt-1 text-sm font-medium lg:mt-0">{room.amenities.length} fasilitas</p><p className="mt-1 line-clamp-1 text-xs text-muted">{room.amenities.map((item) => item.name).join(", ") || "Belum dipilih"}</p></div>
