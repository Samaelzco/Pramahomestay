...
export function RoomFilters({ search, status, type, isActive }: { search?: string; status?: string; type?: string; isActive?: string }) {
  return (
    <form className="mt-8 grid gap-3 border-y py-5 sm:grid-cols-2 xl:grid-cols-[1fr_170px_170px_170px_auto]" method="get">
      ...
      <label><span className="sr-only">Filter inventori</span><select name="is_active" defaultValue={isActive ?? ""} className="h-12 w-full rounded-sm border bg-surface px-4 text-sm outline-none focus:border-primary"><option value="">Semua inventori</option><option value="1">Aktif</option><option value="0">Nonaktif</option></select></label>
      <button type="submit" className="h-12 rounded-sm bg-primary px-6 text-sm font-semibold text-white transition-colors hover:bg-[#2f3131]">Terapkan</button>
    </form>
  );
}
