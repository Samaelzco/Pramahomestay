        ...
        <section>
          ...
          {data.rooms.length ? <div className="mt-6 grid gap-5 lg:grid-cols-2">{data.rooms.map((room) => {
            ...
            return <article key={room.id} className={`overflow-hidden bg-background transition-shadow ${isSelected ? "ring-2 ring-secondary shadow-[0_22px_60px_-42px_rgba(0,0,0,0.55)]" : ""}`} aria-label={`${room.name}${isSelected ? ` · ${serverLocalize(locale, "dipilih", "selected")}` : ""}`}>
              <div className="relative aspect-[16/10] bg-surface-high">{room.images[0] && <Image src={room.images[0].url} alt={room.name} fill sizes="(min-width: 1024px) 35vw, 100vw" unoptimized={shouldBypassImageOptimization(room.images[0].url)} className="object-cover" />}{isSelected && <span className="absolute top-4 right-4 grid size-10 place-items-center rounded-full bg-secondary text-white"><CheckIcon className="size-5" /></span>}</div>
              <div className="p-6"><div className="flex items-start justify-between gap-5"><div><h3 className="text-2xl font-semibold">{room.name}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-muted">{roomDescription(room, locale)}</p></div><p className="shrink-0 text-right font-bold">{money(room.price_per_night, locale)}<span className="block text-xs font-normal text-muted">/{serverLocalize(locale, "malam", "night")}</span></p></div>
                ...
                <div className="mt-6 grid gap-2 sm:grid-cols-[auto_1fr]"><Link href={`/rooms/${room.id}?${keepQuery}`} className="flex min-h-12 items-center justify-center border px-5 text-sm font-semibold transition-colors hover:bg-surface-low">{serverLocalize(locale, "Lihat detail", "View details")}</Link><Link href={`/booking?${hrefQuery}#guest-details`} className={`flex min-h-12 items-center justify-center px-5 text-sm font-bold ${isSelected ? "bg-secondary-soft text-secondary" : "bg-primary text-background"}`}>{isSelected ? serverLocalize(locale, "Kamar dipilih", "Room selected") : serverLocalize(locale, "Pilih kamar", "Select room")}</Link></div>
              </div>
            </article>;
