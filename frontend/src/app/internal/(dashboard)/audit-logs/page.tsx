...
export default async function AuditLogsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const query = {
    ...
    per_page: pageSize(value(params.per_page)), page: value(params.page),
  };
  ...
  const hasFilters = Object.entries(query).some(([key, item]) => key !== "page" && key !== "per_page" && item);
  ...
  return <main className="mx-auto min-h-[calc(100vh-64px)] max-w-[1440px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16">
    ...
    <AuditLogFilters search={query.search} module={query.module} action={query.action} actorId={query.actor_id} dateFrom={query.date_from} dateTo={query.date_to} actors={logs.filter_options.actors} perPage={query.per_page} />
