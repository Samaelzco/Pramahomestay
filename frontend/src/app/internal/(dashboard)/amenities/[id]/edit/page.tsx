...
export default async function EditAmenityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto min-h-[calc(100vh-64px)] max-w-[1200px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16"><Link href="/internal/amenities" className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-secondary underline-offset-4 hover:underline"><ArrowLeftIcon className="size-4" /><LocalizedText id="Kembali ke fasilitas" en="Back to amenities" /></Link><h1 className="mt-8 text-4xl font-semibold tracking-[-0.03em] md:text-5xl"><LocalizedText id={`Edit ${response.data.name}`} en={`Edit ${response.data.name_en || response.data.name}`} /></h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted"><LocalizedText id="Perbarui nama, deskripsi, atau ketersediaan fasilitas." en="Update the amenity name, description, or availability." /></p><AmenityForm amenity={response.data} /></main>;
}
