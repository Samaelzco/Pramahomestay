...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ days?: string; from?: string; to?: string }> }) {
  ...
  const compactCurrency = (value: number) => new Intl.NumberFormat(code, { style: "currency", currency: "IDR", notation: "compact", maximumFractionDigits: 1 }).format(value);
