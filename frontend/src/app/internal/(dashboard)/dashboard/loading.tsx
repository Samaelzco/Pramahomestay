...
export default async function DashboardLoading() {
  ...
  return (
    <main
      aria-busy="true"
      aria-label={serverLocalize(locale, "Memuat ringkasan operasional", "Loading operations overview")}
    ...
    >
      <div className="flex justify-between border-b pb-8">
        <div>
          ...
          <Line className="mt-4 h-5 w-[min(560px,80vw)]" />
        </div>
        <Line className="hidden h-12 w-56 sm:block" />
      </div>
      <div className="grid border-b sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }, (_, index) => (
          <div key={index} className="py-8 sm:px-7">
            <Line className="h-4 w-28" />
            <Line className="mt-4 h-9 w-40 max-w-full" />
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-2">
            {[0, 1].map((item) => (
              <div key={item}>
                ...
                <Line className="mt-8 h-52 w-full" />
              </div>
            ))}
          </div>
        </div>
        <div className="rounded-lg bg-primary p-7">
          <Line className="h-7 w-28 bg-white/20" />
          <Line className="mt-10 h-32 w-full bg-white/10" />
          <Line className="mt-8 h-32 w-full bg-white/10" />
        </div>
      </div>
      <div className="mt-10 grid gap-8 lg:grid-cols-2">
        {[0, 1].map((item) => (
          <Line key={item} className="h-64 w-full rounded-lg" />
        ))}
      </div>
    </main>
  );
