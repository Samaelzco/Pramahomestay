...
export default async function EditBookingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-[1200px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16"><Link href={`/internal/bookings/${booking.id}`} className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-secondary underline-offset-4 hover:underline"><ArrowLeftIcon className="size-4" />{t("Kembali ke detail booking", "Back to booking details")}</Link><h1 className="mt-8 text-4xl font-semibold tracking-[-0.03em] text-primary md:text-5xl">Edit {booking.booking_code}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted">{t("Perbarui profil tamu terpilih, jadwal, kamar, atau status operasional reservasi.", "Update the selected guest profile, schedule, room, or reservation status.")}</p><BookingForm rooms={rooms.data} guests={guests.data} booking={booking} /></main>;
}
