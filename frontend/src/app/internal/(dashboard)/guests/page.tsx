...
export default async function GuestsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const query = { search: value(params.search), page: value(params.page) };
  ...
  return <main className="mx-auto min-h-[calc(100vh-64px)] max-w-[1440px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16">
    ...
    <GuestFilters search={query.search} />
    <div className="mt-8 flex items-baseline justify-between"><p className="text-sm font-medium">{guests.meta.total} tamu ditemukan</p>{query.search && <Link href="/internal/guests" className="text-sm font-semibold text-secondary underline-offset-4 hover:underline">Hapus pencarian</Link>}</div>
