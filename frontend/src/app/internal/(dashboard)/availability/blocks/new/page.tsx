...
export default async function NewRoomBlockPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const requestedRoomId = typeof params.room_id === "string" && /^\d+$/.test(params.room_id) ? Number(params.room_id) : null;
  const initialRoomId = rooms.data.some((room) => room.id === requestedRoomId) ? requestedRoomId : null;
  return <main className="mx-auto min-h-[calc(100vh-64px)] max-w-[1200px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16"><Link href="/internal/availability" className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-secondary underline-offset-4 hover:underline"><ArrowLeftIcon className="size-4" />{t("Kembali ke kalender", "Back to calendar")}</Link><h1 className="mt-8 text-4xl font-semibold tracking-[-0.03em] text-primary md:text-5xl">{t("Blokir kamar", "Block a room")}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted">{t("Tandai kamar yang tidak dapat dijual tanpa membuat booking fiktif.", "Mark a room as unavailable without creating a fake booking.")}</p><RoomBlockForm rooms={rooms.data} initialStart={initialStart} initialRoomId={initialRoomId} /></main>;
}
