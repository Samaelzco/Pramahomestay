...
export default async function UsersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const query = { search: value(params.search), role: value(params.role), is_active: value(params.is_active), per_page: pageSize(value(params.per_page)), page: value(params.page) };
  ...
  return <main className="mx-auto min-h-[calc(100vh-64px)] max-w-[1440px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16">
    ...
    <UserFilters search={query.search} role={query.role} isActive={query.is_active} roles={access.data.roles} perPage={query.per_page} />
