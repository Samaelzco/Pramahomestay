...
export default async function EmailNotificationsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const raw = await searchParams; const query = { search: value(raw.search), status: value(raw.status), type: value(raw.type), page: value(raw.page) };
  const params = new URLSearchParams({ per_page: "20" }); Object.entries(query).forEach(([key, item]) => { if (item) params.set(key, item); });
  ...
  return <main className="mx-auto min-h-[calc(100vh-64px)] max-w-[1440px] px-6 py-10 sm:px-8 md:px-10 md:py-12 xl:px-16">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-4xl font-semibold tracking-[-0.03em] md:text-5xl">{t("Notifikasi email", "Email notifications")}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted">{t("Pantau pembaruan booking dan pembayaran yang dikirim otomatis kepada pelanggan.", "Monitor automated booking and payment updates sent to guests.")}</p></div><Link href="/internal/settings" className="inline-flex min-h-12 items-center justify-center border bg-surface px-5 text-sm font-semibold hover:bg-surface-low">{t("Atur SMTP", "Configure SMTP")}</Link></div>
    <EmailNotificationFilters search={query.search} status={query.status} type={query.type} />
